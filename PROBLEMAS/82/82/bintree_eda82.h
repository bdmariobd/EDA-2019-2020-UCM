...
//1st: tramos. 2: caudal actual. Bool indica si es el primer nodo(para no sumar de mas)
std::pair<int,int> tramos(bintree<int> t, bool padre) {
    if (t.empty()) return { 0,0 };
    if (t.right().empty() && t.left().empty()) return { 0,1 };
    auto dcha = tramos(t.right(),false);
    auto izq = tramos(t.left(),false);
    int aux = std::max(0, dcha.second + izq.second - t.root());
    return { padre? dcha.first + izq.first : aux >= 3 ? dcha.first + izq.first + 1 : dcha.first + izq.first, aux };
}
#endif // bintree_eda_h
