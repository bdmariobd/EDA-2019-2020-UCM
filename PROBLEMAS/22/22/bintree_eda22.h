...
//1st->longitud mayor (sirve para hacer caminos entre el lado derecho y el izquierdo, juntando con el nodo actual)
//2nd->camino mas largo encontrado en ramas abajo, puede que sea mas largo que el formado por el lado dcho e izq
template<typename T>
std::pair<int, int> caminos(const bintree<T>& t){
    if (t.empty()) return { 0,0 };
    auto izq = caminos(t.left());
    auto dcha = caminos(t.right());

    return { 1 + std::max(izq.first, dcha.first), 
            std::max( std::max(izq.second, dcha.second) , 1 + izq.first + dcha.first)
    };
}
template <typename T>
int diametro(const bintree<T> t) {
    if (t.empty()) return 0;
    return caminos(t).second;
}
#endif // bintree_eda_h
