...
int altura(const bintree<T> a)  {
    if (a.empty()) return 0;
    return std::max(altura(a.left()), altura(a.right())) + 1;
}
template <typename T>
int hojas(const bintree<T> a) {
    if (a.empty()) return 0;
    int h = hojas(a.left()) + hojas(a.right());
    if (h == 0) return 1;
    return h;
}
template <typename T>
int nodos(const bintree<T> a) {
    if (a.empty()) return 0;
    return nodos(a.left()) + nodos(a.right())+1;
}
#endif // bintree_eda_h
